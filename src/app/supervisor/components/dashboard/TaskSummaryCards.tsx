...
import type { TaskStatusData, TaskState } from "@/types/task"
...
export function TaskSummaryCards({ data }: TaskSummaryCardsProps) {
  ...
  const mergedData = defaultStatusData.map(defaultItem => {
    const matchingItem = data.find(item => 
      JSON.stringify(item.status.sort()) === JSON.stringify(defaultItem.status.sort())
    )
    return matchingItem ? { ...defaultItem, count: matchingItem.count } : defaultItem
  })
